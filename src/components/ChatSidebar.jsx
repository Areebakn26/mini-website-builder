      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 overflow-x-hidden">
        {messages.map((msg) => (
          <div key={msg.id} className={`flex gap-3 text-xs ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[85%] px-3.5 py-2.5 rounded-2xl break-words whitespace-pre-wrap overflow-hidden ${msg.role === 'user' ? 'bg-violet-600 text-white' : 'bg-slate-800 text-slate-200'}`}>
              {msg.content}
            </div>
          </div>
        ))}
      ...
      <div className="p-3 border-t border-slate-800 bg-slate-900">
        <form onSubmit={(e) => { e.preventDefault(); handleSend(); }} className="relative flex items-center">
          <textarea
            ...
            className={`w-full pl-3 pr-16 py-2.5 bg-slate-950 border rounded-xl text-xs text-slate-200 focus:outline-none resize-none break-words transition ${
